...
def process_wsi(row):
    ...
    for col_idx, (cell_type, cell_id_str) in enumerate(row.items()):
        if col_idx < 1 or not cell_id_str or cell_id_str.strip() == "-":
            continue
        
        # Clean and filter out invalid cell IDs
        cell_id_list = [cid.strip() for cid in str(cell_id_str).split(",") if cid.strip() and cid.strip() != "-"]
        
        if not cell_id_list:
            continue  # Skip if no valid cell IDs remain
        ...
        for cell_id in cell_id_list:
            if cell_id in annotations:
                ...
                with open(label_path, "w") as label_file:
                    class_id = col_idx - 2  # Class starts from column 2 (index 2)
