...
from utils.general import check_img_size, box_iou, non_max_suppression, xyxy2xywh, xywh2xyxy, set_logging, colorstr, scale_coords
    ...
    for file in os.listdir(ndpi_folder):
        if file.endswith('.ndpi'):
            ...
            try:
                ...
                # xml_path= os.path.join(results_folder, wsi + '_predicts.xml')    
...
def run_predict_wsi_multithread(path,model,overlap,tile_size=1024,batch_size = 12):
    ...
    # print('after select', device)  
    torch.cuda.empty_cache()
    half = (globalVars.device != 'cpu')  # half precision only supported on CUDA
    stride = int(model.stride.max())  # model stride
    imgsz = check_img_size(imgsz, s=stride)  # check img_size    

    print('batch_size going in', batch_size)
    ...
    for batch_i, (img, shapes, coords) in enumerate(tqdm(dataloader)):            
        ...
        img = img.half() if half else img.float()  # uint4 to fp16/4        
        ...
        with torch.no_grad():   # Calculating gradients would cause a GPU memory leak
            pred = model(img, augment=globalVars.augment)[0]
            #print('no grad')
        ...
        for i, det in enumerate(pred):  # detections per image                    
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det): 					                    
                    ...
                    # print(label)
                    x1 = coord[0] + (float(line[0]) - float(line[2])/2)*(shapes[i][1])
                    y1 = coord[1] + (float(line[1]) - float(line[3])/2)*(shapes[i][0])
                    x2 = x1 + float(line[2])*shapes[i][1]
                    y2 = y1 + float(line[3])*shapes[i][0]
...
def rerun_predict_pool(wsi_path,model,pred_list,tile_size=1024,batch_size=12):
    ...
    for pred_batch in divide_chunks(pred_list,batch_size):        
        ...
        lefts, tops, img = fetch_batch(image_batch,batch_size)        
...
def fetch_batch(image,batch_size=32):  
    lefts,tops,imgs= [],[],[]
    # print(len(image))
...
def process(wsi_path,pred,tile_size):
    # print(pred)
    x1,y1,x2,y2,label,conf,blah = pred #wsi_path = pred 
    cx = int((x1+x2)/2) 
    cy = int((y1+y2)/2) 
    #breath in pixels        
    breath = abs(x2-x1)
