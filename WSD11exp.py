    ...
    for file in os.listdir(ndpi_folder):
        if file.endswith('.ndpi'):
            ...
            if True:
                ...
                print("the time after first pass:",  (end - start) * 10**3, "ms")
                ...
                pred_list = relabel_uncategorized(pruned_anote_rerun, pruned_cat_anote)

                seg_anote = run_segment(wsi_path,model,pred_list,batch_size=globalVars.batch_size)
                print(seg_anote)
                ...
                anote_xml = write_xml(annotations,start_id,pred_list,X_Reference,Y_Reference,nm_p) 
...
def run_segment(wsi_path,model,pred_list,tile_size=320,batch_size=12):
    ...
    for pred_batch in divide_chunks(pred_list,batch_size):        
        ...
        image_batch = read_tiles_parallel_seg(wsi_path,pred_batch,tile_size)
        # print(len(image_batch))
        lefts, tops, img = fetch_batch_seg(image_batch)        
...
def process_seg(wsi_path,pred):
    ...
    slide = openslide.open_slide(wsi_path)
    level = slide.get_best_level_for_downsample(1.0 / 40)
    tile = slide.read_region((left,top),level,(breadth,height))
    im = tile.convert('RGB')
    img0 = np.array(im)
...
def read_tiles_parallel_seg(slide_path, pred_list, tile_size):
    tiles = []
    try:
        with concurrent.futures.ThreadPoolExecutor(max_workers=4) as executor:
            # Submit the read_tile function for each tile position
            future_to_tile = {executor.submit(process_seg, slide_path, pred, tile_size): pred for pred in pred_list}
            for future in concurrent.futures.as_completed(future_to_tile):
                # tile = future.result()
                tiles.append(future.result())
            # print(executor._max_workers)
...
def fetch_batch_seg(image):  
    lefts,tops,imgs= [],[],[]
    # print(len(image))
    for left,top,img in image:
        lefts.append(left)
        tops.append(top)        
        imgs.append(img)            
    imges = np.concatenate(imgs.copy()) 
