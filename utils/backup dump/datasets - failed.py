...
def create_dataloader_custom(path, imgsz, stride, opt, batch_size, tile_size, overlap, 
                             ...
                             workers=0, image_weights=False, quad=False):  # Set workers=0
    print('batch_size:', batch_size) 
    
    with torch_distributed_zero_first(rank):
        dataset = load_custom(path, imgsz, batch_size=batch_size, tile_size=tile_size, overlap=overlap, stride=int(stride))
        
    batch_size = opt.batch_size  # Ensure batch size consistency
    
    workers = min(os.cpu_count() // world_size, 8)  # Limit to 8 workers
    ...
    dataloader = loader(dataset,
                        ...
                        num_workers=workers,  # Ensure this is 0
                        sampler=sampler,
                        collate_fn=None if quad else load_custom.collate_fn)
    
    return dataloader, dataset


class load_custom(Dataset):
    def __init__(self, path, img_size, tile_size, overlap, batch_size=16, stride=32):
        ...
        # Open WSI once instead of per tile
        ...
        self.level = self.tile_source.level_count - 1

        # Collect tile coordinates once
        cols, rows = self.tile_source.level_tiles[self.level]
    ...
    def __getitem__(self, index):
        ...
        # Load image tile
        img = np.array(self.tile_source.get_tile(self.level, tile_id))  # Keep on CPU
        ...
        return torch.from_numpy(img), self.img_files[index], (h0, w0), coord
    
    def __del__(self):
        """ Close WSI file when dataset is deleted """
        self.slide.close()

    @staticmethod
    def collate_fn(batch):
        """ Standard collate function for dataloader """
        img, indices, shapes, coords = zip(*batch)  
        img = torch.stack(img, 0).to("cuda:0", non_blocking=True)  # Move batch to GPU
        return img, shapes, coords

    @staticmethod
    def collate_fn4(batch):
        """ Special collate function for quad batch processing """
        img, indices, shapes, coords = zip(*batch)
        n = len(shapes) // 4
        ...
        for i in range(n):
            ...
            if random.random() < 0.5:
                im = F.interpolate(img[i].unsqueeze(0).float().to("cuda:0"), scale_factor=2., mode='bilinear', align_corners=False)[0]
            else:
