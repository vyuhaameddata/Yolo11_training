...
current_dir = r'D:\Shreyan\Development\CerviAI'
...
def create_dataloader_custom(path, imgsz, stride, opt, batch_size, tile_size, overlap, 
    ...
    # Ensure only the first DDP process creates the dataset cache first.
    ...
    batch_size = opt.batch_size  # override if needed
    nw = min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers])  # number of workers
    print('nw',nw)
    print('batch_size', batch_size)
    sampler = torch.utils.data.distributed.DistributedSampler(dataset) if rank != -1 else None
    
    # Use DataLoader or InfiniteDataLoader based on image_weights flag
...
class load_custom(Dataset):
    def __init__(self, path, img_size, tile_size, overlap, batch_size=16, single_cls=False, rect=False,
        ...
        # Lazy initialization: these will be created in the worker process when needed.
        self.slide = None
        self.tile_source = None

        # Open slide temporarily to compute tile positions/count.
        slide_tmp = openslide.open_slide(path)
        tile_source_tmp = DeepZoomGenerator(slide_tmp, tile_size=tile_size, overlap=overlap, limit_bounds=False)
        self.level = tile_source_tmp.level_count - 1
        cols, rows = tile_source_tmp.level_tiles[self.level]
        self.img_files = [(col, row) for row in range(rows) for col in range(cols)]
        n = cols * rows  # total number of tiles
        bi = int(n / batch_size)
    ...
    def __getitem__(self, index):
        # Map index (if using shuffling/image_weights, etc.)
        index = self.indices[index]
        # Get the tile (with lazy slide/tile_source init)
        img, orig_hw, resized_hw, coord = self.load_tile(index)
        # Apply letterbox (assuming letterbox is defined elsewhere)
        img, ratio, pad = letterbox(img, self.img_size, auto=False)
        # Convert to CHW and ensure contiguous memory
        img = img.transpose(2, 0, 1)
        img = np.ascontiguousarray(img)
        return torch.from_numpy(img), self.img_files[index], orig_hw, coord

    def load_tile(self, index):
        # Open the slide and tile_source lazily per worker
        if self.tile_source is None:
            self.slide = openslide.open_slide(self.path)
            self.tile_source = DeepZoomGenerator(self.slide, tile_size=self.tile_size, overlap=self.overlap, limit_bounds=False)
        tile_id = self.img_files[index]
        tile = self.tile_source.get_tile(self.level, tile_id)
        img = np.array(tile)
        coord = self.tile_source.get_tile_coordinates(self.level, tile_id)
        h0, w0 = img.shape[:2]
        r = self.img_size / max(h0, w0)  # resize factor
    ...
    def collate_fn(batch):
        # Basic collate: stack images and group other info.
        imgs, indices, shapes, coords = zip(*batch)
        return torch.stack(imgs, 0), shapes, coords

    @staticmethod
    def collate_fn4(batch):
        # Collate that groups four images together (quad mode).
        imgs, indices, shapes, coords = zip(*batch)
        n = len(shapes) // 4
        ...
        for i in range(n):
            ...
            if random.random() < 0.5:
                im = F.interpolate(imgs[idx].unsqueeze(0).float(), scale_factor=2., mode='bilinear', align_corners=False)[0]
                im = im.type(imgs[idx].type())
            else:
                im = torch.cat((torch.cat((imgs[idx], imgs[idx+1]), 1),
                                torch.cat((imgs[idx+2], imgs[idx+3]), 1)), 2)
            img4.append(im)
