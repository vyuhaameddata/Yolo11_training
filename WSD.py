...
class GlobalVars:
    def __init__(self):
        ...
        self.wsi = ''
        self.weights = r'.\abnormal_latest.pt'
        self.source = ''
        self.batch_size = 32
        self.img_size = 640
        self.conf_thres = 0.5
        ...
        self.augment = False
        self.no_trace = True
        self.cat_weights = r'.\cat_latest.pt'
    ...
    for batch_i, (img, shapes, coords) in enumerate(tqdm(dataloader)):            
        ...
        for i, det in enumerate(pred):  # detections per image                    
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):  # Process detections
                    # Extract coordinates directly from the tensor
                    x1 = coord[0] + float(xyxy[0])
                    y1 = coord[1] + float(xyxy[1])
                    x2 = coord[0] + float(xyxy[2])
                    y2 = coord[1] + float(xyxy[3])

                    # Prepare annotation and append
